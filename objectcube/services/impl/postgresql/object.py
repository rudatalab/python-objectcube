...
class ObjectService(BaseObjectService):
    ...
    def update(self, object):
        ...
        if object.id is None or not isinstance(object.id, LongType):
            raise ObjectCubeException('Unable to update Object without id')
        if not object.name or not isinstance(object.name, UnicodeType):
            raise ObjectCubeException('Unable to update Object without a valid name')
        if not object.digest or not isinstance(object.digest, UnicodeType):
