...
class DimensionService(BaseDimensionService):
    ...
    def _construct_tree(self, dimension_node_array, counter=None):
        ...
        while True:
            if counter[0] >= len(dimension_node_array) \
                    or dimension_node_array[counter[0]].right_border \
                    > parent_node.right_border:
                return parent_node
            parent_node.child_nodes.append(
                self._construct_tree(dimension_node_array, counter))
    ...
    def _read_roots(self, tag=None):
        ...
        if tag is not None:
            sql = 'SELECT D1.root_tag_id, D1.node_tag_id, T1.value,' \
                  'D1.left_border, D1.right_border ' \
                  'FROM Dimensions D1 ' \
    ...
    def _delete(self, subtree_root_node):
        ...
        # Output: None
        sql = 'DELETE FROM DIMENSIONS WHERE root_tag_id = %s ' \
              'AND left_border >= %s AND right_border <= %s RETURNING *'
        params = (subtree_root_node.root_tag_id,
                  subtree_root_node.left_border,
                  subtree_root_node.right_border)
    ...
    def delete(self, subtree_root_node):
        ...
        if root is None:
            raise ObjectCubeException('Must give DimensionNode '
                                      'which has a root')

        # Node must be in the tree, so find it and delete it
        node = self._find_node(root, subtree_root_node.node_tag_id)
        if node is None:
            raise ObjectCubeException('Must give DimensionNode which is in '
                                      'the tree of the root')
        self._delete(node)
